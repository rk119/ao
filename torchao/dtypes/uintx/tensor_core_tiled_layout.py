    ...
    def from_plain(
        ...
        def quant_2d(int_data_2d):
            if TORCH_VERSION_AT_LEAST_2_5:
                int_data_2d = (int_data_2d[::, ::2] << 4 | int_data_2d[::, 1::2]).to(torch.uint8)
            return torch.ops.aten._convert_weight_to_int4pack(
                int_data_2d, _layout.inner_k_tiles
            )
        if int_data.dim() == 3: # for moe quant
            ...
            zero_point = zero_point.reshape(int_data.shape[0], int_data.shape[-2], -1) if zero_point is not None else None
        else:
            assert int_data.dim() == 2
            packed_weight = quant_2d(int_data)
            scale = scale.reshape(int_data.shape[0], -1)
            zero_point = zero_point.reshape(int_data.shape[0], -1) if zero_point is not None else None
