...
from torchao.quantization.quant_api import AOBaseConfig, register_quantize_module_handler, dataclass
...
class FakeExtraDimTensor(torch.Tensor):
    ...
    def __new__(
        ...
        for tensor in tensors:
            assert tensor.dtype==dtype, f"all tensors in FakeExtraDimTensor must have same dtype but got {tensor.dtype} and {dtype}"
            assert tensor.shape==shape, f"all tensors in FakeExtraDimTensor must have same shape but got {tensor.shape} and {shape}"
            assert tensor.device == device, f"all tensors in FakeExtraDimTensor must have same device but got {tensor.device} and {device}"
            assert tensor.layout == layout, f"all tensors in FakeExtraDimTensor must have same layout but got {tensor.layout} and {layout}"
        kwargs = {}
        kwargs["dtype"] = dtype
    ...
    def __init__(
        ...
        if len(tensors) == 0 and tensor_tail is not None:
            tensors = [tensor_tail.head_tensor,]
    ...
    def __tensor_flatten__(self):
        if self.tensor_tail is None:
            return ["head_tensor",], [self.num_tensors]
        else:
            return ["head_tensor", "tensor_tail",], [self.num_tensors]
    ...
    def __torch_function__(cls, func, types, args, kwargs=None):
        ...
        if func is torch.nn.functional.linear:
            ...
            assert w.num_tensors == 1, "FakeExtraDimTensor used in a linear op when it had multiple tensors"
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs):
        ...
        if func == aten.slice.Tensor:
            ...
            if dim==0:
                return return_and_correct_aliasing(
                    ...
                    args,
                    kwargs,
                    cls(self.tensor_list[start:end:step])
                )
        ...
        try:
            return return_and_correct_aliasing(
                func, args, kwargs, args[0]._apply_fn_to_data(
